...
export async function run(): Promise<void> {
  try {
    ...
    if(!authResponse.ok) {
      throw new Error(`Failed to authenticate with Portainer: ${authResponse.statusText}`)
    }
    const authData: any = await authResponse.json()
    const token = authData.jwt
    core.setSecret(token)
    const containersResponse = await fetch(`${portainerHost}/api/endpoints/3/docker/v1.41/containers/json?all=true`, {
      headers: {
    ...
    const container = containersData.find((container: any) => container.Names.includes(`/${containerName}`))
    if(!container) {
